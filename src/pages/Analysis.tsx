...
export default function Analysis({
  ...
  const handleAddJobUrl = () => {
    if (jobUrls.length < 5) setJobUrls([...jobUrls, ""]);
  };
  const handleRemoveJobUrl = (i: number) => {
    if (jobUrls.length > 1) setJobUrls(jobUrls.filter((_, idx) => idx !== i));
  };
  const handleUpdateJobUrl = (i: number, v: string) => {
    const u = [...jobUrls];
    u[i] = v;
    setJobUrls(u);
  };
  ...
  const handleStartAnalysis = async () => {
    if (!githubUsername) {
      setError(
        "GitHub 계정이 설정되지 않았습니다. 마이페이지에서 GitHub 계정명을 먼저 등록해주세요.",
      );
      return;
    }
    if (!resumeText.trim()) {
      setError("이력서 텍스트를 입력하거나 파일을 업로드해 주세요.");
      return;
    }
    ...
    try {
      const data = await api.analyzeResumeGithub(
        resumeText,
        null,
        githubUsername,
        [],
      ...
      <div className="flex flex-col gap-5">
        ...
        >
            ...
            <svg width="72" height="72" viewBox="0 0 36 36">
              ...
              <path
                ...
                style={{
                  stroke: "#22C55E",
                  strokeWidth: "3",
                  strokeDasharray: `${overall_match_pct}, 100`,
                }}
                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
              />
            </svg>
          ...
          <div className="flex-1">
            ...
            <div className="text-[10px] text-zinc-500">
              기술일치 · 커밋활동 · 레포커버리지 각 33.3% 동일 가중치
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="card">
            ...
            <p className="text-[10px] text-zinc-400 mt-2 m-0">
              GitHub 기술 ↔ 이력서 기술 일치율
            </p>
          </div>
          ...
          <div className="card">
            ...
            <MetricBar
              pct={Math.round((active_weeks / 52) * 100)}
              color="#2563EB"
            />
            <p className="text-[10px] text-zinc-400 mt-2 m-0">
              총 커밋 {total_commits}회 · 최근 52주 활동
            </p>
          </div>
        ...
        <div className="card">
          <div className="flex justify-between items-center border-b border-zinc-100 pb-3 mb-4">
            ...
            <span className="badge badge-info">
              {github_analysis.repo_count}개 레포 스캔
            </span>
          </div>
          <div className="grid grid-cols-4 gap-3 mb-4">
            ...
            <div className="bg-zinc-50 rounded-lg p-3">
              ...
              <span className="text-xs font-semibold text-zinc-900">
                {github_analysis.total_commits}회
              </span>
            </div>
            <div className="bg-zinc-50 rounded-lg p-3">
              <span className="text-[10px] text-zinc-400 block mb-1">
          ...
          <div className="mb-4">
            <span className="block text-[10px] font-bold text-zinc-500 mb-2 uppercase tracking-wide">
              감지된 기술 스택
            </span>
            <div className="flex flex-wrap gap-1.5">
              {github_analysis.tech_stack.map((t) => (
          ...
          <div className="flex flex-col gap-2">
            {github_analysis.repo_details.map((repo) => (
              <div
                key={repo.url}
                className="border border-zinc-100 rounded-xl p-3 hover:border-zinc-200 transition-colors"
              >
                ...
                {repo.description && (
                  <p className="text-[11px] text-zinc-500 m-0 mb-1.5">
                    {repo.description}
                  </p>
                )}
                <div className="flex items-center gap-3 text-[10px] text-zinc-400">
                  ...
                  <span
                    className={`${repo.readme_status.startsWith("미흡") ? "text-red-400" : repo.readme_status === "보통" ? "text-amber-400" : "text-emerald-500"}`}
                  >
          ...
          {github_analysis.readme_suggestions?.length > 0 && (
            <div className="mt-4 pt-4 border-t border-zinc-50">
              ...
              {github_analysis.readme_suggestions.map((s, i) => (
                <div
                  key={i}
                  className="flex gap-2 text-xs text-zinc-600 leading-relaxed mb-1.5"
                >
                  <AlertTriangle
                    size={12}
        ...
        <div className="card">
          ...
          <div className="bg-zinc-50 rounded-xl p-3 flex gap-3 items-start mb-4">
            ...
            <div>
              <h4 className="m-0 text-xs font-bold text-zinc-900 mb-0.5">
                이력서 품질 코멘트
              </h4>
              <p className="m-0 text-[11px] text-zinc-500 leading-relaxed">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="text-xs font-bold mb-1.5 block text-emerald-600">
                ✓ 교차 검증 성공
              </span>
              <div className="flex flex-wrap gap-1.5">
                {resume_analysis.verified_skills.map((s) => (
            ...
            <div>
              <span className="text-xs font-bold mb-1.5 block text-amber-600">
                ✗ 코드 증명 필요
              </span>
              <div className="flex flex-wrap gap-1.5">
                {resume_analysis.unverified_skills.length > 0 ? (
                  resume_analysis.unverified_skills.map((s) => (
        ...
        <div className="card">
          ...
          <div className="bg-red-50 border border-red-100 rounded-xl p-3 mb-4">
            <span className="block text-[10px] font-bold text-zinc-600 mb-2 uppercase tracking-wide">
              미보유 보완 기술
            </span>
            <div className="flex flex-wrap gap-1.5">
              {skill_gap.missing_technologies.map((t) => (
          ...
          <div className="flex flex-col gap-4">
            {recommended_projects.map((proj, i) => (
              <div key={i} className="border border-zinc-100 rounded-xl p-4">
                ...
                <p className="text-xs text-zinc-500 leading-relaxed m-0 mb-3">
                  {proj.description}
                </p>
                ...
                <div className="bg-white border border-zinc-100 rounded-lg py-2 px-3 mb-3">
                  ...
                  <p className="text-[10px] text-zinc-500 leading-relaxed m-0">
                    {proj.architecture}
                  </p>
                </div>
  ...
  const EmptyPlaceholder = () => (
    ...
    >
      ...
      <p className="text-xs text-zinc-400 leading-relaxed max-w-xs">
        ...
        <strong style={{ color: "#16A34A" }}>분석 시작</strong> 버튼을
        눌러주세요.
      </p>
      ...
      <div className="mb-6">
        ...
        <div
          ...
          onFocusCapture={(e) => {
            ...
            e.currentTarget.style.boxShadow = "0 4px 24px rgba(22,163,74,0.14)";
          }}
          onBlurCapture={(e) => {
          ...
          >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                onBlur={(e) => {
                  if (e.target.value.trim()) validateResumeText(e.target.value);
                }}
              />
              {validating && (
                <p className="text-[11px] text-zinc-400 flex items-center gap-1">
                  <Loader2 size={11} className="animate-spin" /> 이력서 확인
                  중...
                </p>
              )}
